...
// Эта инструкция выполняется в FS-ветке (r65=0x06) при нажатой Fn
ADDR(0x05B7)[] = { 0x02, 0x31, 0x00 };  // LJMP 0x3100

// Отладочная функция по адресу 0x3100 (свободная область)
// При Fn+F7 (код 0xFE) отправляет символ "O" (KC_O = 0x12) по USB
// При любом другом коде — возвращается к обычной обработке FS-клавиши
ADDR(0x3100)[] = {
    // Проверка: A != 0xFE → переход к нормальной обработке
    0xB4, 0xFE, 0x13,        // CJNE A, #0FEh, NORMAL

    // === A == 0xFE (Fn+F7) → отправить символ "O" ===
    0x7D, 0x12,              // MOV R5, #12h          ; R5 = KC_O
    0x7F, 0x01,              // MOV R7, #01h          ; R7 = 1 (нажатие)
    0x12, 0x1B, 0xFF,        // LCALL L0374           ; поместить в USB-буфер
    0x12, 0x17, 0x0D,        // LCALL L0143           ; отправить по USB
    0x7F, 0x00,              // MOV R7, #00h          ; R7 = 0 (отпускание)
    0x12, 0x1B, 0xFF,        // LCALL L0374           ; поместить release
    0x12, 0x17, 0x0D,        // LCALL L0143           ; отправить release
    0x22,                    // RET

    // === NORMAL: A != 0xFE → обычная обработка FS-клавиши ===
    0x02, 0x05, 0xC3,        // LJMP 0x05C3 (L0429)
};